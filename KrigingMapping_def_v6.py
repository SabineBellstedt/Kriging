...
# # Galaxies = ['NGC6010', 'NGC6014', 'NGC6798', 'UGC06062', 'NGC3595']
# b_a = {}
...
# Reff = {}
# for GalName in Galaxies:
#   Reff_Spitzer[GalName] = 10**np.array(cat['log_Re_'])[np.where(np.array(cat['Gal']) == GalName)]
...
def KrigingMapPython(inputPath, namegal, genTable, label='Z', limits=[-3, +2], visualize=False,
                      savePDF=True, sizePixelMap=80, contours=True, points=True, ellipses=True, ellipses_rotated = False, simulated = False):
  #Retrieving galaxy parameters' dictionary
  #Creating the Kriging maps with Python
  #reading input file
  fileKriging = asciidata.open(inputPath+'gridKrig_'+label+'.txt')
  ...
  if points:
    ...
    rangeZmap = [numpy.max([minZmap, limits[0]]), 
               numpy.min([maxZmap, limits[1]])]
...
#   Isophotes
    ...
    if ellipses:
      ...
      ells = [Ellipse(xy=[0,0], width=(2.*jj*Reff_Spitzer[namegal]/numpy.sqrt(b_a[namegal])), 
                  height=(2.*jj*Reff_Spitzer[namegal]*numpy.sqrt(b_a[namegal])), angle=90-PA0[namegal], 
                  edgecolor = 'k', facecolor = 'none', fill = False, linestyle = 'dashed') for jj in radiuses]
      for ee in ells: 
        ax.add_artist(ee)
    elif ellipses_rotated:
      radiuses = numpy.array([1,2, 3, 4, 5])
      ells = [Ellipse(xy=[0,0], width=(2.*jj*Reff_Spitzer[namegal]/numpy.sqrt(b_a[namegal])), 
                  height=(2.*jj*Reff_Spitzer[namegal]*numpy.sqrt(b_a[namegal])), angle=0, 
                  edgecolor = 'k', facecolor = 'none', fill = False, linestyle = 'dashed') for jj in radiuses]
      for ee in ells: 
        ax.add_artist(ee)
#   Contours
...
#Points
    ...
    if label == 'Z':
      ...
      cb.set_label(r"$V_{rms}$ [km/s]")

    if simulated:
      ax.set_title(str(namegal))
    else:
...
def radialProfileLog(namegal, inputFile, label='Z', #binsize=50,  #Bin numerosity
  ...
  ellDist = findDell(xK, yK, PA0[namegal], b_a[namegal])
  ellDist_Sorted = ellDist[permutation_indices(ellDist)]
  ellDist_Sorted_log = numpy.log10(ellDist[permutation_indices(ellDist)]/Reff_Spitzer[namegal])
  zK_Sorted = zK[permutation_indices(ellDist)]
  errzK_Sorted = errzK[permutation_indices(ellDist)]
  #
  # Limit elements within datapoints
  ...
  if datapoints != []:
    RA_dp, Dec_dp = numpy.array(datapoints)[:, 0], numpy.array(datapoints)[:, 1]
    ellDist_dp_log = numpy.log10(findDell(RA_dp, Dec_dp, PA0[namegal], b_a[namegal])/Reff_Spitzer[namegal])
...
def MCerrors(linear_prof_R, log_prof_R, totRealizations, namegal, genTable, rangeKriging, label='Z'):
  ...
  # LINEAR
  #
  linear_prof_errp, linear_prof_errm, linear_prof_median = [], [], []
  n_elements_linear = []
  for jj in numpy.arange(len(linear_prof_R)):
    ...
    linear_prof_median.append(numpy.percentile(tmpValues, 50))
    n_elements_linear.append(len(tmpValues))
  #
  # LOGARITHMIC
  #
  log_prof_errp, log_prof_errm, log_prof_median = [], [], []
  n_elements_log = []
  for jj in numpy.arange(len(log_prof_R)):
    ...
    log_prof_median.append(numpy.percentile(tmpValues, 50))
    n_elements_log.append(len(tmpValues))
  #
  return (numpy.array(linear_prof_errm), numpy.array(linear_prof_errp), numpy.array(linear_prof_median), n_elements_linear,
          array(log_prof_errm), numpy.array(log_prof_errp), numpy.array(log_prof_median), n_elements_log)


#To create bootstrap realizations
def bootstrapRealization2(genTable_vel, genTable_sig, pathOutput, realization): #Input is table to give Kriging
  import random
  from random import choice
  lines_vel, lines_sig = [], []
  for jj in genTable_vel:
    lines_vel.append(jj)
  for jj in genTable_sig:
    lines_sig.append(jj)
  #
  #Shuffling
  #
  newList_vel, newList_sig = [], []
  Selection = []
  for jj in numpy.arange(len(lines_vel)):
    random.seed()
    # select = random.choice(lines)
    select = random.randint(0, len(lines_vel)-1)
    # To avoid duplicates, if the line already exists, the positions RA and Dec are
    # offset by a random value in the range -0.5<D<0.5 arcsec.
    # if select in numpy.array(newList):
    if select in Selection:
      lines_vel[select][0] += random.random()-0.5
      lines_vel[select][1] += random.random()-0.5
    #
    Selection.append(select)
    # print select
    # print lines_vel[select]
    if len(lines_vel[select]) == 4: # for the velocity dispersion we apply the same altered coordinates as for velocity. 
      newList_vel.append([lines_vel[select][0],lines_vel[select][1],lines_vel[select][2],lines_vel[select][3]])
      newList_sig.append([lines_vel[select][0],lines_vel[select][1],lines_sig[select][2],lines_sig[select][3]])
    else:
      newList_vel.append([lines_vel[select][0],lines_vel[select][1],lines_vel[select][2]])
      newList_sig.append([lines_vel[select][0],lines_vel[select][1],lines_sig[select][2]])
#
  newList_vel = numpy.array(newList_vel)
  newList_sig = numpy.array(newList_sig)
# Save in dir
  if not(os.path.exists(pathOutput+'/MC'+str(realization))):
    os.mkdir(pathOutput+'/MC'+str(realization))
# Savetxt file
  listTmp = []
  for jj in newList_vel:
    listTmp.append('\t'.join(map(str, jj))) #Join elements of the same line
  fileTMP = open(pathOutput+'/MC'+str(realization)+'/realization_'+str(int(realization))+'_Points_vel.txt', 'wb')
  fileTMP.write("\n".join(listTmp))
  fileTMP.close()
  listTmp = []
  for jj in newList_sig:
    listTmp.append('\t'.join(map(str, jj))) #Join elements of the same line
  fileTMP = open(pathOutput+'/MC'+str(realization)+'/realization_'+str(int(realization))+'_Points_sig.txt', 'wb')
  fileTMP.write("\n".join(listTmp))
  fileTMP.close()
  return True

def MCOutputs(totRealizations, namegal, genTable_vel, genTable_sig, theta):
  #
  ## create realizations
  list_R_MC_lin, list_Val_MC_lin = [], []
  list_R_MC_log, list_Val_MC_log = [], []
  if not(os.path.exists(namegal+'/MC_Errors')):
    os.mkdir(namegal+'/MC_Errors')
  #
  # check whether realisations already exist from a different parameter. We should use the same ones for 
  for jj in numpy.arange(totRealizations):
    if verbose:
      stdout.write("\rMC Realization n. %i / %i" % (jj+1, totRealizations))
      stdout.flush()
    #
    dummy = bootstrapRealization2(genTable_vel, genTable_sig, namegal+'/MC_Errors', jj)
  ## run kriging R on all the realizations
  pbar = ProgressBar(widgets=[Percentage(), Bar(marker='*')], maxval=(totRealizations)).start()
  for jj in numpy.arange(totRealizations):
    dummy = KrigingR(namegal+'/MC_Errors/MC'+str(jj)+'/realization_'+str(jj)+'_Points_vel.txt', visualize=False,
         theta_r = int(theta), coeff_r = 3, savePdf = False,
         pathOutput = namegal+'/MC_Errors/MC'+str(jj)+'/', label='vel')
    dummy = KrigingR(namegal+'/MC_Errors/MC'+str(jj)+'/realization_'+str(jj)+'_Points_sig.txt', visualize=False,
         theta_r = int(theta), coeff_r = 3, savePdf = False,
         pathOutput = namegal+'/MC_Errors/MC'+str(jj)+'/', label='sig')
    time.sleep(0.001)
    pbar.update(jj+1)
  pbar.finish()

def MCOutputs_Extended(totRealizations, namegal, genTable_vel, genTable_sig, theta):
  #
  ## create realizations
  list_R_MC_lin, list_Val_MC_lin = [], []
  list_R_MC_log, list_Val_MC_log = [], []
  if not(os.path.exists(namegal+'/MC_Errors_Extended')):
    os.mkdir(namegal+'/MC_Errors_Extended')
  #
  # check whether realisations already exist from a different parameter. We should use the same ones for 
  for jj in numpy.arange(totRealizations):
    if verbose:
      ...
      stdout.flush()
    #
    dummy = bootstrapRealization2(genTable_vel, genTable_sig, namegal+'/MC_Errors_Extended', jj)
  ## run kriging R on all the realizations
  pbar = ProgressBar(widgets=[Percentage(), Bar(marker='*')], maxval=(totRealizations)).start()
  for jj in numpy.arange(totRealizations):
    dummy = KrigingR(namegal+'/MC_Errors_Extended/MC'+str(jj)+'/realization_'+str(jj)+'_Points_vel.txt', visualize=False,
         theta_r = int(theta), coeff_r = 3, savePdf = False,
         pathOutput = namegal+'/MC_Errors_Extended/MC'+str(jj)+'/', label='vel')
    dummy = KrigingR(namegal+'/MC_Errors_Extended/MC'+str(jj)+'/realization_'+str(jj)+'_Points_sig.txt', visualize=False,
         theta_r = int(theta), coeff_r = 3, savePdf = False,
         pathOutput = namegal+'/MC_Errors_Extended/MC'+str(jj)+'/', label='sig')
    time.sleep(0.001)
    pbar.update(jj+1)
  pbar.finish()
